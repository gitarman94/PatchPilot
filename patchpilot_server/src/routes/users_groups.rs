...
pub fn list_users_groups(user: AuthUser, pool: &State<DbPool>) -> Template {
    ...
    let joined: Vec<(i32, i32, String)> = users_dsl::users
        .inner_join(user_groups_dsl::user_groups.on(users_dsl::id.eq(user_groups_dsl::user_id)))
        .select((user_groups_dsl::group_id, users_dsl::id, users_dsl::username))
        .load(&mut conn)
        .unwrap_or_default();
...
pub fn add_group(user: AuthUser, pool: &State<DbPool>, form: Form<GroupForm>) -> Redirect {
    ...
    let _ = db_log_audit(&mut conn, &user.username, "group.create", Some(&ff.group_name), ff.description.as_deref());
...
pub fn delete_group(user: AuthUser, pool: &State<DbPool>, group_id_val: i32) -> Redirect {
    ...
    let _ = diesel::delete(user_groups_dsl::user_groups.filter(user_groups_dsl::group_id.eq(group_id_val))).execute(&mut conn);
    let _ = diesel::delete(groups_dsl::groups.filter(groups_dsl::id.eq(group_id_val))).execute(&mut conn);
...
pub fn delete_user(user: AuthUser, pool: &State<DbPool>, user_id_val: i32) -> Redirect {
    ...
    let _ = diesel::delete(user_groups_dsl::user_groups.filter(user_groups_dsl::user_id.eq(user_id_val))).execute(&mut conn);
    let _ = diesel::delete(users_dsl::users.filter(users_dsl::id.eq(user_id_val))).execute(&mut conn);
