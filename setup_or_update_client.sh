...
# Prompt user for server IP (mandatory)
while true; do
  ...
  if [[ -n "$SERVER_IP" ]]; then
    ...
    echo "Server IP cannot be empty. Please enter a valid IP."
  fi
done

echo "[*] Creating config.json with provided server IP..."
cat > "$CONFIG_PATH" <<EOF
{
  "server_ip": "$SERVER_IP",
  "client_id": ""
...
echo "[*] Enabling and starting patchpilot_client service..."
