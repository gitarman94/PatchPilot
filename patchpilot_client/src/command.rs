...
pub async fn execute_command_and_post_result(
    ...
    let cmd_id = cmd_item.get("id").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
    let cmd_string = cmd_item.get("exec")
        .and_then(|v| v.as_str())
        .or_else(|| cmd_item.get("script").and_then(|v| v.as_str()))
        .unwrap_or("");

    if cmd_string.is_empty() {
        let _ = post_command_result(&client, &server_url, &device_id, &cmd_id,
            json!({"status":"error","reason":"missing exec/script field"})
        ).await;
        return;
    }
    ...
    let run = task::spawn_blocking(move || {
        #[cfg(windows)]
        let out = std::process::Command::new("cmd").args(&["/C", &cmd_string]).output();

        #[cfg(not(windows))]
        let out = std::process::Command::new("sh").arg("-c").arg(&cmd_string).output();
        ...
        match out {
            ...
            Err(e) => (false, String::new(), format!("Failed to start process: {}", e), -1)
        }
    }).await;
    ...
    match run {
        ...
        Err(e) => {
            ...
            let _ = post_command_result(&client, &server_url, &device_id, &cmd_id,
                json!({"status":"error","reason":format!("panic: {}", e)})).await;
        }
    }
}
                ...
                if let Ok(val) = resp.json::<Value>().await {
                    if let Some(arr) = val.as_array() {
                        for cmd_item in arr {
                            ...
                            tokio::spawn(async move { execute_command_and_post_result(c, s, d, ci).await });
                        }
                    }
                }
            },
